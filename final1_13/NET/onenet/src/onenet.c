...
#define PROID			"5KgqTM02HK"

#define ACCESS_KEY		"SEdxZ0M4dmpNM0Q1dndRTkVYc1JZaEs2ak02SkdXUHg="
...
extern u8 temp,humi,light,hwsend,warning_temp,dianji_flag;
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"hwsend\":{\"value\":%d},", tag);
	strcat(buf, text);
    
	//rgb初始
	memset(text, 0, sizeof(text));
	sprintf(text, "\"RGB_flag\":{\"value\":%d},", 0);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"warning_temp\":{\"value\":%d},", 0);
	strcat(buf, text);
    ...
    memset(text,0,sizeof(text));
    sprintf(text, "\"dianji\":{\"value\":%s},",  dianji_flag? "true" : "false");
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"warning\":{\"value\":%s}", warning ? "true" : "false");
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	char numBuf[10];
	int num = 0;
	//加入warning标志，warning开启则开启警报功能
	cJSON *raw_json, *params_json, *led_json, *beep_json, *rgb_json, *warning_json, *warning_temp_json,*dianji_json;
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息
			...
			if(result == 0)
			{
				...
				dianji_json=cJSON_GetObjectItem(params_json,"dianji");
				if(dianji_json!=NULL)
				{
						if(dianji_json->type==cJSON_True)
				...
				{
					if(warning_json->type==cJSON_True) warning = 1;
					else warning = 0;
				}
				warning_temp_json=cJSON_GetObjectItem(params_json,"warning_temp");
				if(warning_temp_json!=NULL)
				{
					warning_temp = warning_temp_json->valueint;
				}
				rgb_json=cJSON_GetObjectItem(params_json,"RGB_flag");
				if(rgb_json!=NULL)
				{
						int num = rgb_json->valueint;  // 
				...
				cJSON_Delete(raw_json);
					...
					if(data_ptr)
					{
						...
						OneNET_Publish(topic_buf, "ok");										//回复命令
					}
				}
